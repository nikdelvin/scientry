...
export default function Button({
    ...
    const pointerStyle = (() => {
        if (isLoading) return 'pointer-events-none'
        else if (isDisabled)
            return `pointer-events-none opacity-50 ${type === 'solid' ? 'bg-zinc-950/30' : 'bg-zinc-50/10'}`
        else return ''
    })()
    const isIconOnlyStyle = {
        aspectRatio: isIconOnly ? 'aspect-square w-auto h-auto' : 'w-full',
        buttonSize: isIconOnly ? 'p-2' : 'px-4 py-2'
    }

    return (
        <div
            className={`relative inline-flex max-w-[300px] items-center justify-center overflow-hidden rounded-full bg-gradient-to-r p-[1px] ${isIconOnlyStyle.aspectRatio} ${textSize} ${borderGD[color]} ${shadowType}`}
        >
            <div
                ...
                {type === 'solid' ? (
                    ...
                    >
                        <motion.button
                            ...
                            onClick={onClick}
                            className={`inline-flex w-full select-none appearance-none items-center justify-center gap-2 overflow-hidden whitespace-nowrap rounded-full bg-transparent antialiased ${isIconOnlyStyle.buttonSize} ${pointerStyle}`}
                        >
                            {children}
                        </motion.button>
                    </div>
                    ...
                    <div className={`inline-flex w-full items-center justify-center rounded-full bg-zinc-950`}>
                        ...
                        >
                            <motion.button
                                ...
                                transition={{ duration: 0.2 }}
                                onClick={onClick}
                                className={`inline-flex w-full select-none appearance-none items-center justify-center gap-2 overflow-hidden whitespace-nowrap rounded-full bg-transparent antialiased ${isIconOnlyStyle.buttonSize} ${textColor[color]} ${pointerStyle}`}
                            >
                                {children}
                            </motion.button>
                        </div>
